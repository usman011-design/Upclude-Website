...
export function SidebarNavigation({ activeIndex, onNavigate }: SidebarNavigationProps) {
  ...
  return (
    <div className="fixed left-3 sm:left-6 md:left-12 top-1/2 z-40 flex -translate-y-1/2 flex-col gap-3 sm:gap-4 md:gap-6">
      {navItems.map((item, index) => (
        <button
          key={index}
          onClick={() => onNavigate(index)}
          className={`group relative flex h-8 w-8 sm:h-9 sm:w-9 md:h-10 md:w-10 items-center justify-center rounded-full transition-all duration-300 ${
            activeIndex === index
